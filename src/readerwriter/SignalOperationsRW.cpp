...
//--------------------------------------------------------------
void saveInto(JsonValue& root, ImageOperationCollection& collection, const ImageOperationConnections& co)
{
    Rect boxed = collection.getBoundingBox();
    auto& jsonOp = root.setPath("texture-nodal","operations");
    auto& ops = collection.operations;
    int index = 0;
    for(auto it = ops.begin(); it != ops.end(); ++it)
    {
        auto& jNode = jsonOp.setPath(index++);
        std::string typeName = qb::getImageOperationName(static_cast<qb::ImageOperationType>(it->second->nodeTypeId()));
        jNode.setPath("type").set(typeName);
        jNode.setPath("id").set((float)it->first);
        toJson(jNode.setPath("position"), it->second->position-boxed.p0);
        ...
        for(size_t i=0; i<op->getPropertyCount(); ++i)
        {
            ...
            int k = 0;
            bool b = false;
            if (op->getPropertyType(i) == ImageDataType_Float)
            {
                op->getProperty(i, f);
                jNode.setPath("properties", op->getPropertyName(i)).set(f);
            }
            else if (op->getPropertyType(i) == ImageDataType_Int)
            {
                op->getProperty(i, k);
                jNode.setPath("properties", op->getPropertyName(i)).set((float)k);
            }
            else if (op->getPropertyType(i) == ImageDataType_Bool)
            {
                op->getProperty(i, b);
                jNode.setPath("properties", op->getPropertyName(i)).set(b);
            }
    ...
    auto& jsonCo = root.setPath("texture-nodal","connections");
    int coIndex = 0;
    for (auto entry : co.entries)
    {
        jsonCo.setPath(coIndex, "src").set(entry.src);
...
//--------------------------------------------------------------
void loadFrom(JsonValue& root, ImageOperationCollection& collection, ImageOperationConnections& co)
{
    auto& jsonOp = root.setPath("texture-nodal","operations");
    auto& ops = collection.operations;
    int index = 0;
    for(auto& jNode : jsonOp.array.values)
    {
        ...
        vec2 position;
        jsonTo(jNode.setPath("position"), position);
        std::unique_ptr<ImageNode> ptr;
        ptr.reset( Factory<ImageNode>::create(qb::getImageOperationType(type)) );
    ...
    for(auto& jco : jsonCo.array.values)
    {
        ImageOperationConnections::Entry entry;
        entry.src = (int) jco.setPath("src").getNumeric();
        entry.src_index = (int) jco.setPath("src-pin").getNumeric();
        entry.dst = (int) jco.setPath("dst").getNumeric();
        entry.dst_index = (int) jco.setPath("dst-pin").getNumeric();
